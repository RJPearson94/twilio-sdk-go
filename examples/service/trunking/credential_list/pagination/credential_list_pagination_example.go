...
func main() {
	paginator := trunkingClient.
		...
		CredentialLists.
		NewCredentialListsPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v credential list(s) found on page %v", len(currentPage.CredentialLists), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of credential list(s) found: %v", len(paginator.CredentialLists))
}
