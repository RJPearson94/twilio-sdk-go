...
func main() {
	...
	for paginator.Next() {
		...
		log.Printf("%v sync map item(s) found on page %v", len(currentPage.Items), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of sync map item(s) found: %v", len(paginator.Items))
}
