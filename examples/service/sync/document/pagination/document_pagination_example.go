...
func main() {
	...
	for paginator.Next() {
		...
		log.Printf("%v document(s) found on page %v", len(currentPage.Documents), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of document(s) found: %v", len(paginator.Documents))
}
