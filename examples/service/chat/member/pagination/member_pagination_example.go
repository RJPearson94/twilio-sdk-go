...
func main() {
	paginator := chatSession.
		...
		Members.
		NewChannelMembersPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v member(s) found on page %v", len(currentPage.Members), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of member(s) found: %v", len(paginator.Members))
}
