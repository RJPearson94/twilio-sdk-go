...
func main() {
	paginator := autopilotClient.
		...
		FieldValues.
		NewFieldValuesPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v field value(s) found on page %v", len(currentPage.FieldValues), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of field value(s) found: %v", len(paginator.FieldValues))
}
