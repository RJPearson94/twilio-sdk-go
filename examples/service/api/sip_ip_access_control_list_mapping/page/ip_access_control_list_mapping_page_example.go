...
func main() {
	resp, err := apiClient.
		...
		Page(&ip_access_control_list_mappings.IpAccessControlListMappingsPageOptions{})

	if err != nil {
		log.Panicf("%s", err.Error())
	}

	log.Printf("%v IP access control list mapping(s) found on page", len(resp.IpAccessControlListMappings))
}
