...
func main() {
	...
	for paginator.Next() {
		...
		log.Printf("%v media attachment(s) found on page %v", len(currentPage.Media), currentPage.Page)
	}
	...
	log.Printf("Total number of media attachment(s) found: %v", len(paginator.Media))
}
