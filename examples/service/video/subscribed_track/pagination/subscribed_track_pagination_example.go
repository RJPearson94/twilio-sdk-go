...
func main() {
	paginator := videoClient.
		...
		SubscribedTracks.
		NewSubscribedTracksPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v subscribed track(s) found on page %v", len(currentPage.SubscribedTracks), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of subscribed tracks(s) found: %v", len(paginator.SubscribedTracks))
}
