...
func main() {
	paginator := serverlessSession.
		...
		Deployments.
		NewDeploymentsPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v deployment(s) found on page %v", len(currentPage.Deployments), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of deployment(s) found: %v", len(paginator.Deployments))
}
