...
func main() {
	paginator := studioSession.
		...
		Executions.
		NewExecutionsPaginator()

	for paginator.Next() {
		currentPage := paginator.CurrentPage()
		log.Printf("%v executions(s) found on page %v", len(currentPage.Executions), currentPage.Meta.Page)
	}
	...
	log.Printf("Total number of executions(s) found: %v", len(paginator.Executions))
}
